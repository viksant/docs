...
BestChatBot reads your store through the **WooCommerce REST API**. You generate a read-only API key in WordPress, then paste it into BestChatBot.

### Step 1 — Generate a WooCommerce REST API key

In WordPress, go to **WooCommerce → Settings → Advanced → REST API** and click **Add key**.

<Frame>
  <img src="/integrations/wordpress-integration/WP-api-key-generation-1.png" alt="WooCommerce Settings Advanced tab with the REST API keys section" />
</Frame>

Give it a **Description**, pick a **User**, and set **Permissions** to **Read** (the bot never writes to your store). Click **Generate API key**.

<Frame>
  <img src="/integrations/wordpress-integration/WP-api-key-generation-2.png" alt="WooCommerce create API key form with Description, User, and Permissions set to Read" />
</Frame>

WooCommerce shows your **Consumer key** (`ck_...`) and **Consumer secret** (`cs_...`). Copy both now — the secret is only shown once.

<Frame>
  <img src="/integrations/wordpress-integration/WP-api-key-generation-3.png" alt="Generated WooCommerce Consumer key and Consumer secret" />
</Frame>

### Step 2 — Connect in BestChatBot

In your workspace dashboard, open the **Integrations** tab, find **WooCommerce**, and click **Connect**. In the popup:

- **User name** → your **Consumer key** (`ck_...`)
- **Password** → your **Consumer secret** (`cs_...`)
- **Domain** → your store's domain **without** `https://` (e.g., `my-store.com`)

<Frame>
  <img src="/integrations/wordpress-integration/wordpress-integration-from-dashboard.png" alt="BestChatBot WooCommerce connection popup with User name, Password, and Domain fields" />
</Frame>
...
You'll see a success confirmation, and WooCommerce's actions become available right away. If Shopify was connected, it's disconnected automatically.

<Frame>
  <img src="/integrations/wordpress-integration/wordpress-integration-successfuly-connected-dashboard.png" alt="WooCommerce integration connected successfully confirmation" />
</Frame>

From the integration panel you can toggle each **bot capability** — **Search products** and **Track order**.

<Frame>
  <img src="/integrations/wordpress-integration/integration-connected-options.png" alt="Connected WooCommerce panel with Search products and Track order capability toggles" />
</Frame>
